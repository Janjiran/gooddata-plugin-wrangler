...
export default function addPlugin(
    ...
    const output = execSync(`cross-env GDC_USERNAME=${gdcUsername} GDC_PASSWORD=${gdcPassword}  npx @gooddata/plugin-toolkit dashboard-plugin add "${pluginUrl}" --backend ${backendType} --hostname "${backendUrl}" --workspace-id "${workspaceId}"`, {encoding: 'utf-8'});
